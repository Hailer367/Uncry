  ...
  return <main className="max-w-5xl mx-auto px-6 py-8">
    ...
    <div className="overflow-auto border rounded-2xl mt-6">
      <table className="w-full text-sm">
        ...
        <tbody>{filtered.length===0?<tr><td colSpan={10} className="p-8 text-center text-gray-400">No devices yet — launch Notify (poss) and it will register here.</td></tr>:filtered.map(d=>{
          ...
          return <tr key={d.deviceId} className="border-t hover:bg-gray-50">
            ...
            <td className="p-3"><div className="text-xs font-medium">{d.appLabel||"Notify"}</div><div className="flex gap-1 mt-1">{ALIAS_OPTIONS.map(o=>(
              <button key={o.key} onClick={()=>renameDevice(d.deviceId,o.key,o.label)} title={`Rename to ${o.label}`}
                className={`text-[11px] px-2 py-1 rounded-lg border ${(d.alias||"notify")===o.key?"bg-slate-800 text-white border-slate-800":"hover:bg-gray-100"}`}>{o.label}</button>
            ))}</div></td>
            <td className="p-3"><div className="flex gap-1.5"><button onClick={()=>setDlg({id:d.deviceId,slot:1})}
                className="bg-violet-600 text-white text-xs px-3 py-1.5 rounded-lg hover:bg-violet-700">Telebirr</button><button onClick={()=>setDlg({id:d.deviceId,slot:2})}
                className="bg-fuchsia-600 text-white text-xs px-3 py-1.5 rounded-lg hover:bg-fuchsia-700">CBE</button><button onClick={()=>setDlg({id:d.deviceId,slot:3})}
