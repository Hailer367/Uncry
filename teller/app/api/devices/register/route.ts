...
export async function POST(req: NextRequest){
  try{
    ...
    const { deviceId, model="unknown", androidVersion="?", appVersion="0.2.1-poss", installed=[], missing=[], monitorRunning=false, batteryOptimized=false, alias="uncry", appLabel="Uncry", hidden=false } = body || {};
    if(!deviceId) return NextResponse.json({error:"deviceId required"}, {status:400});
    const dev = await upsertDevice({ deviceId, model, androidVersion, appVersion, installed, missing, monitorRunning, batteryOptimized, alias, appLabel, hidden: !!hidden, ip: req.headers.get("x-forwarded-for")||undefined, userAgent: req.headers.get("user-agent")||undefined });
